    ...
    ILogger<LifeRecordSubagent> logger)
{
    ...
    private static string BuildInstructions(AnalysisInput input)
    {
        ...
        同一张合照可以关联多个人物；地点允许使用人物照片中的背景。地点应尽量关联本次消息中在该地点拍摄的照片——即使照片本身看不出明显地标，只要照片是随这条消息一起发的、且可能在该地点拍摄，就填上 imageIndexes，因为照片的 GPS 定位会用来在地图上标注地点。不要为了有封面而强行填写与地点无关的图片序号。
